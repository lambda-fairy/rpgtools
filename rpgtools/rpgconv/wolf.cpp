...
void Iconv::open(const char *to, const char *from)
{
    iconv_close(cd);
    cd = iconv_open(to, from);
}

std::string Iconv::conv(const std::string &str)
{
    //Prepare variables
    size_t inSize = str.size();
    size_t outSize = inSize * 4;
    char *in = const_cast<char*>(str.data());
    std::vector<char> outbuf(outSize);
    char *out = outbuf.data();

    //Convert and null-terminate
    if (iconv(cd, &in, &inSize, &out, &outSize) == (size_t)-1)
        throw std::runtime_error("codepage conversion error");

    //Return string
    return std::string(outbuf.data(), out - 1);
}

//iconv does things differently than windows; correct these tiny differences
const char *quirks_cp932[6][2] = {
    {"\u301C", "\uFF5E"},
    {"\u2016", "\u2225"},
    {"\u2212", "\uFF0D"},
    {"\u00A2", "\uFFE0"},
...
std::string fromJis(const std::string &string)
{
    if (string.empty())
        return "";
#if defined OS_W32
    //Convert to wide
    int size = MultiByteToWideChar(932, 0, string.data(), string.size(), NULL, 0);
    if (size <= 0)
        throw std::runtime_error("codepage conversion error");

    std::vector<wchar_t> wbuffer(size);
    if (MultiByteToWideChar(932, 0, string.data(), string.size(), wbuffer.data(), wbuffer.size()) != size)
        throw std::runtime_error("codepage conversion error");

    //Convert to UTF-8
    size = WideCharToMultiByte(CP_UTF8, 0, wbuffer.data(), wbuffer.size(), NULL, 0, NULL, NULL);
    if (size <= 0)
        throw std::runtime_error("codepage conversion error");

    std::vector<char> buffer(size);
    if (WideCharToMultiByte(CP_UTF8, 0, wbuffer.data(), wbuffer.size(), buffer.data(), buffer.size(), NULL, NULL) != size)
        throw std::runtime_error("codepage conversion error");

    return std::string(buffer.begin(), buffer.end());
#else
    static Unix::Iconv iconv(UTF8, "cp932");
    std::string ustr = iconv.conv(string);
    //Replace chars with Microsoft versions
    for (unsigned int i = 0; i < ARRAY_SIZE(Unix::quirks_cp932); ++i) {
        size_t len = std::strlen(Unix::quirks_cp932[i][0]);
        for (;;) {
            size_t index = ustr.find(Unix::quirks_cp932[i][0]);
            if (index == std::string::npos)
                break;
            ustr.replace(index, len, Unix::quirks_cp932[i][1]);
        }
    }
    return ustr;
#endif
